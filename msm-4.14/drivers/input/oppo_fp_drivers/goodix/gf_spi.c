...
static int gf_vreg_setup(struct gf_dev *gf_dev,const char *name,
                bool enable)
{
        int rc;
        ...
        if (enable) {
                if (!vreg) {
                        vreg = regulator_get(&gf_dev->spi->dev, name);
                        if (IS_ERR(vreg)) {
                ...
                if (regulator_count_voltages(vreg) > 0) {
						pr_err("start to set voltage \n");
                        rc = regulator_set_voltage(vreg,2800000,3300000);
                        if (rc) {
                                pr_err("Unable to set voltage on %s, %d\n",name, rc);
                        }
                }

                rc = regulator_set_load(vreg, 20000);
...
#endif
{
	...
	gf_dev->irq_enabled = 1;
	gf_disable_irq(gf_dev);
	status = gf_vreg_setup(gf_dev, "ldo5", enable);
	if (status) {
		pr_err("gf_vreg_setup failed\n");
	}
