...
static void sub_mainboard_verify(struct devinfo_data *devinfo_data)
{
        ...
        devinfo_data->sub_chan = iio_channel_get(&devinfo_data->devinfo->dev, "sub_adc_gpio3");
        if (IS_ERR(devinfo_data->sub_chan)) {
            pr_err("sub_chan get error.\n");
            devinfo_data->sub_chan = NULL;
        } else {
            ret = iio_read_channel_processed(devinfo_data->sub_chan, &sub_volt);
            if (ret < 0) {
                pr_err("read sub_chan volt failed(%d).\n", ret);
            } else {
                sub_volt = sub_volt/1000;
                pr_err("read sub_chan volt %d.\n", sub_volt);
            }
        }
sub_mainboard_set:
        ...
        operate = get_Operator_Version();   //inner(0.25-0.39),foreign fullband(0.4-0.5), foreign no fullband(0.51-0.67), carrier(0.68-0.8)
        if ((OPERATOR_CHINA_MOBILE == operate) || (OPERATOR_ALL_CHINA_CARRIER == operate)) {
                if ((sub_volt >= 250) && (sub_volt <= 400)) {
                        snprintf(mainboard_info.manufacture, INFO_BUF_LEN, "%d-%d-match", get_project(), sub_volt);
                } else {
                        snprintf(mainboard_info.manufacture, INFO_BUF_LEN, "%d-%d-unmatch", get_project(), sub_volt);
                }
        } else if ((OPERATOR_FOREIGN_ASIA == operate) || (OPERATOR_FOREIGN_ASIA_SIMPLE == operate)) {
                if (((sub_volt >= 570) && (sub_volt <= 690)) || ((sub_volt >= 410) && (sub_volt <= 520))){
                        snprintf(mainboard_info.manufacture, INFO_BUF_LEN, "%d-%d-match", get_project(), sub_volt);
                } else {
                        snprintf(mainboard_info.manufacture, INFO_BUF_LEN, "%d-%d-unmatch", get_project(), sub_volt);
                }
        } else if (OPERATOR_FOREIGN_FULL_BAND == operate) {
                if ((sub_volt >= 410) && (sub_volt <= 520)) {
                        snprintf(mainboard_info.manufacture, INFO_BUF_LEN, "%d-%d-match", get_project(), sub_volt);
                } else {
                        snprintf(mainboard_info.manufacture, INFO_BUF_LEN, "%d-%d-unmatch", get_project(), sub_volt);
                }
        } else if (OPERATOR_FOREIGN_CARRIER == operate) {
                if ((sub_volt >= 700) && (sub_volt <= 810)) {
                        snprintf(mainboard_info.manufacture, INFO_BUF_LEN, "%d-%d-match", get_project(), sub_volt);
                } else {
                        snprintf(mainboard_info.manufacture, INFO_BUF_LEN, "%d-%d-unmatch", get_project(), sub_volt);
                }
        } else {
                snprintf(mainboard_info.manufacture, INFO_BUF_LEN, "%d-%d", get_project(), operate);
        }
