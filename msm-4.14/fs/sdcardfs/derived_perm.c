...
//Jiemin.Zhu@PSW.Android.SdardFs, 2017/12/12, Add for sdcardfs delete dcim record
	ci->data->under_extra = pi->data->under_extra;
#endif /* VENDOR_EDIT */
}
...
//Jiemin.Zhu@PSW.Android.SdardFs, 2017/12/12, Add for sdcardfs delete dcim record
	...
	struct qstr q_coloros_ocloud = QSTR_LITERAL(".ColorOSGalleryOcloud");
#endif /* VENDOR_EDIT */
...
//Jiemin.Zhu@PSW.Android.SdardFs, 2017/12/12, Add for sdcardfs delete dcim record
		} else if (qstr_case_eq(name, &q_coloros_recycler) || qstr_case_eq(name, &q_coloros_ocloud)) {
			info->data->under_extra = true;
			set_top(info, parent_info);
#endif /* VENDOR_EDIT */
		} else {
			set_top(info, parent_info);
